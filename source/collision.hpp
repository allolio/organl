...
class TriangleIntersects
{
  ...
  private:
    ...
    // vertex matching: compare is v1 matches with any of the rest
    inline static bool sameVertex (const TemplatedVec &v1, const TemplatedVec &u1, const TemplatedVec &u2, const TemplatedVec &u3,  double &debug_reduced_to_0__) {
        ...
        conditionally_round_to_zero(min);  // if (fabs(min) < EPSILON) min = 0.0;
...
//            throw std::logic_error("cannot normalize a zero-lenght vector");
    ...
    inline static bool same_vertex(const TemplatedVec &V0, const TemplatedVec &U0, const size_t i0, const size_t i1)
    {
        return ((abs(V0[i0] - U0[i0]) < EPSILON) and (abs(V0[i1] - U0[i1]) < EPSILON));
    }

    inline static bool shared_vertices(const std::array<TemplatedVec,3> &V, const std::array<TemplatedVec,3> &U,
                                       const size_t i0, const size_t i1, bool & collide)
    {
        bool common = false;
        std::array<bool,3> commonU { {false, false, false} };
        std::array<bool,3> commonV { {false, false, false} };

        for (int i = 0; i < 3; i ++)
        {
            for (int j = 0; j < 3; j ++)
            {
                if (same_vertex(V[i], U[j], i0, i1))
                {
                    common = true;
                    commonU[i] = true;
                    commonV[j] = true;
        ...
        if (a < 0)  // the triangles are same
        {
            collide = true;
            return true;
        }

        if (point_in_tri(V[a], U[0], U[1], U[2], i0, i1))
        {
            collide = true;
            return true;
        }

        if (point_in_tri(U[a], V[0], V[1], V[2], i0, i1))
        {
            collide = true;
            return true;
        }
  ...
  public:
                                                 ...
                                                 bool check_isect_endpoints, double &debug_reduced_to_0__)
    {
        ...
        /* allowing equality solves vertex-vertex collisions */
        if (isect2[0] - isect1[1] > -EPSILON || isect1[0] - isect2[1] > -EPSILON)
            return false;                                      // RETURN

        /* allowing non-planar edge-edge collisions */  /* TODO:commit */
        /* Here, the triangles are already non-planar, therefore if 2 vertices of one triangle match the intersection point
         * they are not colliding */
        if (sameVertex(isectpointA1, V0, V1, V2, debug_reduced_to_0__) && sameVertex(isectpointA2, V0, V1, V2, debug_reduced_to_0__)) {
            return false;
        }
                                 ...
                                 const TemplatedVec &U2)
    {
        ...
        bool collide = false;
        std::array<TemplatedVec,3> V {};
        std::array<TemplatedVec,3> U {};
        for (int i = 0; i < 3; i ++)
        {
            V[0][i] = V0[i];
            V[1][i] = V1[i];
        ...
        if (shared_vertices(V, U, i0, i1, collide))
            return collide;
