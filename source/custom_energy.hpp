...
public:
    ...
    double tray;
    double stored;
    map<string, double> props; // This to store, e.g. Geodesic curvature if necessary.
};
...
public:
    void setTriangle(NTriangle *n) { nt=n; v1=nt->v[edgeindex];}
    ...
    bool restore() override
    {
        ...
        for(int i=0;i!=edgep->deps.size();i++)
        {
            if(edgep->deps[i]->ptype->collidable)
            {
                ...
                if(edgep->deps[i]->content==(void*) nt) {found=true;  break;}
            }
        }
...
double crosstan(Property *e)
{
    ...
    // Eval my own tan at x=1;
        if(edgeindex==2)
        {           dist=*nt->v[2]-*nt->v[0]; norm=*nt->n[2];}
        else 
    ...
    // Eval the other tan at x=0;
      ...
      int edgeind2=e->state;
        ...
        else dist=*nt2->v[edgeind2+1]-*nt2->v[edgeind2];
    ...
    t1=t1/t1.abs(); t2=t2/t2.abs();
    // Find orientation.;
    int sgn=1; if(norm*(t1%t2)<0) sgn=-1;
...
double evol(const double x, double *l)
    {
        ...
        if(edgeindex==2)
            dist=*nt->v[2]-*nt->v[0];
        else dist=*nt->v[edgeindex+1]-*nt->v[edgeindex];
        ...
        {
            t=dist+ nt->c[edgeindex]*(2*x-1) + (nt->c[edgeindex+3])*(3*x*x-1);
            dt=nt->c[edgeindex]*(2.0) + (nt->c[edgeindex+3])*(6*x);
        }
       dtds=1./t.abs(); //CHeck this;
       if(edgeindex==0) norm=nt->normal(x,0); // x 0
       else if(edgeindex==1) norm=nt->normal(0,x); // 0 x
       else norm=nt->normal(x,x); // x x
       // https://people.math.wisc.edu/~angenent/561/kgsolutions.pdf
       *l=1./dtds;// 
       return dt*(norm%t)*(dtds*dtds);//*dtds); // Geodesic c// kg Last one removed due to line element. --> kg dl
      //  else     
...
class BonnetEdgeFactory : public CustomDepEnergyFactory
{
    ...
    CustomDepEnergy* GetNew(void *parent)
    {
        ...
        for(int i=0;i!=ep->deps.size();i++)
        {
            ...
            {bne->setTriangle((NTriangle*) ep->deps[i]->content);  break;}
        }
        bne->next=NULL;
        return bne;
    }
...
public:
    void setTriangle(NTriangle *n) { nt=n; edgeindex=edgeIndexDetect(); }
     void setParent(EdgeEnerProperty *eep) {ep=eep;}
...
int edgeIndexDetect()
	{
		...
		FaceVertexMesh *fvm=(FaceVertexMesh*) ((Property*) (ep->parent))->ptype->parent;
		Edge *edge=(Edge*) ((Property*) (ep->parent))->content;
		v1=&fvm->vertices[edge->vertA].pos;
		v2=&fvm->vertices[edge->vertB].pos;
		int redgeindex=-1;
		if ( nt->v[0] == v1 and nt->v[1]== v2) redgeindex=0;
		if ( nt->v[0] == v2 and nt->v[1]== v1) redgeindex=0;
                if ( nt->v[1] == v1 and nt->v[2]== v2) redgeindex=1;
                if ( nt->v[2] == v2 and nt->v[1]== v1) redgeindex=1;
                if( redgeindex==-1) redgeindex=2;
		return redgeindex;
	}
...
double evol(const double x)
    {
       ...
       // https://people.math.wisc.edu/~angenent/561/kgsolutions.pdf
       // 
       return 1./dtds;//*dtds); // Geodesic c// kg Last one removed due to line element. --> kg dl
       //  else     
